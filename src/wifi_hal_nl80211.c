...
bm_sta_list_t *steering_add_stalist(wifi_interface_info_t *interface, char *ssid, mac_address_t client_mac, uint8_t type)
{
    ...
    if (bm_client_info == NULL) {
        ...
        hash_map_put(interface->bm_sta_map, key, bm_client_info);
    }
    if (ssid) {
...
static rate_limit_entry_t *wifi_hal_rate_limit_entry_get(mac_address_t mac)
{
    ...
    hash_map_put(g_wifi_hal.mgt_frame_rate_limit_hashmap, mac_str, entry);
             ...
             void *valid_finish_data)
{
    ...
    if (!nl_info) {
        if ((nl_info = create_nl80211_socket())) {
            hash_map_put(g_wifi_hal.netlink_socket_map, thread_id, nl_info);
        }
    }
...
int interface_info_handler(struct nl_msg *msg, void *arg)
{
    ...
    if (tb[NL80211_ATTR_WIPHY]) {
        ...
        if (radio != NULL && tb[NL80211_ATTR_IFNAME]) {
            ...
            hash_map_put(radio->interface_map, interface->name, interface);
...
int init_nl80211()
{
    ...
    hash_map_put(g_wifi_hal.netlink_socket_map, thread_id, core_thread_socket);
