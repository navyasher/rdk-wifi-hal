...
EC_POINT *dpp_build_point_from_connector_string(wifi_device_dpp_context_t *ctx, const char *connector)
{
	...
	ptr++;
    snprintf(connector_encoded, sizeof(connector_encoded), "%s", ptr); //CID 96
...
wifi_dpp_session_data_t *create_dpp_reconfig_session_instance(wifi_device_dpp_context_t *ctx)
{
	...
	if (fd != -1) {
		size_t bytes_read = 0;
		bytes_read = read(fd, &data->u.reconfig_data.tran_id[0], sizeof(data->u.reconfig_data.tran_id[0]));
		if (!bytes_read) {
			close(fd);
			return NULL;
		}
...
wifi_dppReconfigInitiate(wifi_device_dpp_context_t *ctx)
{
	...
	if (fd != -1) {
		size_t bytes_read = 0;
		bytes_read = read(fd, &data->u.reconfig_data.tran_id[retry_cnt], sizeof(data->u.reconfig_data.tran_id[retry_cnt]));
		if (!bytes_read) {
			close(fd);
			return RETURN_ERR;
		}
